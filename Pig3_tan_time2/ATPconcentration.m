...
%         Mtotal(j) = Q_total(j) *  (Sa - Sv); 
    ...
    switch MetabolicDrive
        ...
        case 'VariableSV'
            ...
            test.endo.MetSignal(j) = Sa - max(test.endo.Sv(j),0);
            test.mid.MetSignal(j)  = Sa - max(test.mid.Sv(j),0);
            test.epi.MetSignal(j)  = Sa - max(test.epi.Sv(j),0);
            
        case 'Generic'
            
            test.endo.MetSignal(j) = Sa - test.endo.dS(j);
            test.mid.MetSignal(j)  = Sa - test.mid.dS(j);
            test.epi.MetSignal(j)  = Sa - test.epi.dS(j);
